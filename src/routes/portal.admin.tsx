          ...
          <div className="space-y-4 rounded-lg border border-dashed border-border/60 bg-card/40 p-5 text-center">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Create one to assign this job to.
              </p>
            </div>
            <Button
              type="button"
...
function InlineCreateClient({
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="space-y-1.5">
        <Label htmlFor="icemail">Client email</Label>
        <Input
          id="icemail"
          type="email"
          required
      ...
      <div className="space-y-1.5">
        <Label htmlFor="icpwd">Password (leave blank to auto-generate)</Label>
        <Input
          id="icpwd"
          type="text"
          minLength={8}
      ...
      <DialogFooter className="gap-2">
        ...
        <Button type="submit" disabled={busy}>
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Create & select"}
        </Button>
      </DialogFooter>
    </form>
  );
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (mode === "signin") {
      ...
      const { error } = await supabase.auth.signUp({
        email,
        password,
        options: { emailRedirectTo: `${window.location.origin}/portal/admin` },
      });
      if (error) toast.error(error.message);
    ...
    <div className="mx-auto flex min-h-[80vh] max-w-md flex-col justify-center px-6">
      <div className="mb-8 text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          {mode === "signin" ? "Sign in to manage workspaces." : "Create your admin account."}
        </p>
      </div>
      <form
        onSubmit={submit}
